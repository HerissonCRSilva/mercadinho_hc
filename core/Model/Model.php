 ...
 */
use MercadinhoHC\core\model\Database;
use \PDO;
...
abstract class Model extends Database {
    ...
    private function fieldsJoin($data = null) {
        if (empty($data) && isset($this->join['fields'])) {
            return implode(',' . $this->join['table'] . '.', $this->join['fields']);
        }
        ...
        if (!empty($data)) {
            ...
            return implode(',' . $this->join['table'] . '.', $fields);
        }
    ...
    private function where() {
        return $this->where = (isset($this->data['conditions'])) ? 'WHERE ' . self::conditions(' AND ') : '';
    }
    ...
    private function find() {
        ...
        if (empty($this->join)) {
            ...
            $sql = "SELECT " . self::fields() . ", " . self::fieldsJoin() . " FROM {$this->table} " . $this->join['type'] . " JOIN " . $this->join['table'] . " ON "  . $this->join['condition'] . " " . self::where();
        }
